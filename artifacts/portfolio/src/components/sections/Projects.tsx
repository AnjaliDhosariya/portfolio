...
const PROJECTS: Project[] = [
  ...
  },
  {
    ...
    overview: "A computer vision application that translates sign language gestures into readable text in real-time. Designed to bridge the communication gap and improve accessibility for the hearing and speech impaired.",
    bullets: [
      "Real-time video feed processing and gesture tracking",
      "Machine learning models trained on custom sign language datasets",
      "Instantaneous translation from gestures to text",
      "Intuitive and accessible user interface",
    ],
    outcomes: [
      { metric: "Real-time", label: "Translation speed" },
      { metric: "Accessibility", label: "Breaks communication barriers" },
      { metric: "Custom ML", label: "Trained for high accuracy" },
    ],
    arch: {
      ...
      steps: [
        {
          label: "Video Input",
          desc: "Capture live video feed from the user's webcam",
        },
        {
          label: "Feature Extraction",
          desc: "Detect hand landmarks and track gestures frame-by-frame",
        },
        {
          label: "Classification",
          desc: "Pass extracted features through the trained ML model",
        },
        {
          label: "Text Output",
          desc: "Map the classified gesture to its corresponding text and display",
        },
      ],
    },
    tech: ["Python", "Computer Vision", "Machine Learning"],
    links: { github: "https://github.com/AnjaliDhosariya/signLanguageTranslation" },
  },
];
    ...
    <div className="flex flex-col gap-1.5">
      {steps.map((step, i) => (
        <div key={i} className="flex flex-col">
          <div className="flex items-start gap-3 bg-card/60 border border-border/40 rounded-lg px-3 py-2.5">
...
function ProjectDialog({
  ...
  return (
    ...
    >
      <DialogContent
        className="max-w-2xl w-full bg-card border border-border/60 p-0 overflow-hidden rounded-2xl max-h-[90vh] flex flex-col"
                ...
                <div>
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    {project.outcomes.map((o, i) => (
                      <div
                        key={i}
                        className="bg-card/60 border border-border/40 rounded-lg px-3 py-3 text-center"
                      >
      ...
      <div className="container mx-auto px-6 max-w-6xl">
        ...
        >
          <h2 className="text-3xl md:text-4xl font-mono font-bold mb-3 flex items-center gap-4">
            <span className="text-secondary text-xl">04.</span> Some Things I've
            Built
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {PROJECTS.map((project, idx) => (
            <motion.div
              ...
              className="bg-card border border-border rounded-xl p-6 flex flex-col cursor-pointer hover:border-primary/50 hover:shadow-[0_0_24px_rgba(255,0,160,0.15)] transition-all duration-500 group"
            >
